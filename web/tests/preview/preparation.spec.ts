...
test('manual multi-turn input survives refresh and publishes Dataset v1', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '移除此轮', exact: true }).nth(1).click()
  await page.getByRole('button', { name: '保留此轮', exact: true }).click()
  await expect(page.getByLabel('第 2 轮输入', { exact: true })).toHaveValue('金额改成 90 万，期限不变')
  await page.getByRole('button', { name: '移除此轮', exact: true }).nth(1).click()
  await page.getByRole('button', { name: '确认移除', exact: true }).click()
  await expect(page.getByLabel('第 2 轮输入', { exact: true })).toHaveCount(0)
  await page.getByRole('button', { name: '添加对话轮次', exact: true }).click()
  await page.getByLabel('第 2 轮输入', { exact: true }).fill('金额改成 90 万，期限不变')
  await page.getByLabel('第 2 轮期望', { exact: true }).fill('更新金额并保留期限')
...
test('rule trial, validation and version upgrades preserve evaluator and historical run snapshots', async ({
  ...
  await expect(page.getByLabel('版本说明（发布时必填）', { exact: true })).toHaveValue('校验新规则')
  expect((await stored(page)).evaluators.find(item => item.id === 'ev-rule')!.versions).toHaveLength(2)
  await page.getByRole('button', { name: '发布新版本', exact: true }).click()
  await page.getByRole('button', { name: '确认发布', exact: true }).click()
...
test('LLM and composite single-sample outcomes separate NA, resource errors and short-circuited children', async ({
  ...
  await expect(page.locator('.prep-result')).toContainText('分数无分数')
  await page.getByRole('button', { name: '编辑并发布新版本', exact: true }).click()
  const removeChild = page.getByRole('button', { name: '移除子项', exact: true })
  const childCount = await removeChild.count()
  await removeChild.first().click()
  await page.getByRole('button', { name: '保留子项', exact: true }).click()
  await expect(removeChild).toHaveCount(childCount)
  await removeChild.first().click()
  await page.getByRole('button', { name: '确认移除', exact: true }).click()
  await expect(removeChild).toHaveCount(childCount - 1)
})
