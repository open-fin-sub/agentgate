...
test('real comparison uses server deltas, exposes counts, and preserves both evidence paths', async ({
  ...
  await page.getByRole('link', { name: '查看基线证据', exact: true }).first().click()
  await expect(page).toHaveURL(new RegExp(`/runs/${baseline}/cases/`))
  await expect(page.getByText(/评分标准固定版本/)).toBeVisible()
  await page.getByRole('link', { name: '← 返回版本对比与筛选', exact: true }).click()
  await expect(page).toHaveURL(/change=improvement/)
  await page.getByRole('link', { name: '查看候选证据', exact: true }).first().click()
  await expect(page).toHaveURL(new RegExp(`/runs/${candidate}/cases/`))
})
...
test('real forward and reverse lineage preserve exact content and reach reports', async ({
  ...
  await expect(page.getByRole('heading', { name: '关联测评任务', exact: true })).toBeVisible()
  await page.goto(`/runs/${baseline}`)
  await page.getByRole('link', { name: '此对象版本的关联任务', exact: true }).click()
  await expect(page).toHaveURL(/kind=target/)
  expect(new URL(page.url()).searchParams.get('hash')).toBe(
