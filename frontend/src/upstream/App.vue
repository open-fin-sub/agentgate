...
import DatasetWorkspace from '../views/datasets/index.vue';
...
const caseNames = computed(() =>
  Object.fromEntries((report.value?.run.manifest.dataset.cases ?? []).map((c) => [c.id, c.name])),
);
const failed = computed(
  () => report.value?.results.filter((item) => item.outcome === 'fail') ?? [],
);
const traceTurns = computed(() =>
  Object.entries(trace.value?.turn_outcomes ?? {}).map(([turnId, outcome]) => ({
    turnId,
    ...outcome,
  })),
);
const selectedAgent = computed(() =>
  versions.value.find((item) => item.id === selectedVersion.value),
);
const selectedDatasetInfo = computed(() =>
  datasets.value.find((item) => item.id === selectedDataset.value),
);
...
async function refresh() {
  ...
  if (selectedEvaluators.value.length === 0)
    selectedEvaluators.value = evaluatorOptions.map((item) => item.id);
...
async function openTrace(caseId: string) {
  ...
  trace.value = await api.trace(report.value.run.id, caseId);
...
const asPercent = (score: number | null) =>
      ...
      <main v-if="page === 'evaluate'">
        <section class="region config-region" aria-labelledby="config-title">
          ...
          <div class="config-grid">
            ...
            <article class="config-card">
              ...
              <p>
                ...
                {{ selectedDatasetInfo?.case_count ?? 0 }} 个用例
              </p>
            </article>

            <article class="config-card evaluator-card">
              <div class="card-index">E</div>
              <label>Evaluators & Metrics</label>
              ...
              <el-checkbox-group v-model="selectedEvaluators" class="evaluator-list">
                <el-checkbox v-for="item in evaluators" :key="item.id" :value="item.id" border>
                  ...
                  ><small>{{ item.metric }} · {{ item.dimension }}</small>
                </el-checkbox>
              </el-checkbox-group>
            </article>
          </div>
          ...
          <div class="launch-bar">
            <div>
              <b>{{ selectedEvaluators.length }}</b> 个评估器已启用 <span>· 结果将写入 SQLite</span>
            </div>
            <el-button
              type="primary"
              size="large"
        ...
        <section id="result-report" class="region report-region" aria-labelledby="report-title">
          <div class="region-heading report-heading">
            <div>
              <span class="step">02 · RESULT REPORT</span>
              <h2 id="report-title">结果报告</h2>
              <p v-if="report">
              ...
              <p v-else>运行评估后在此查看指标、失败证据和轨迹。</p>
            </div>
            <el-tag
              v-if="report"
              :type="report.release_gate.outcome === 'pass' ? 'success' : 'danger'"
              effect="dark"
              size="large"
              >{{
                report.release_gate.outcome === 'pass' ? '发布门槛通过' : '发布门槛未通过'
              }}</el-tag
            >
          </div>
          ...
          <template v-if="report">
            <div class="metric-grid" aria-label="评估指标">
              ...
              >
                <span>{{ metricLabel(metric.key) }} · {{ metric.level }}</span
                ><strong>{{ asPercent(metric.score) }}</strong>
                <el-progress
                  :percentage="Math.round((metric.score ?? 0) * 100)"
                  :show-text="false"
                  :stroke-width="7"
                  :color="(metric.score ?? 0) >= 0.95 ? '#20b486' : '#e85d75'"
                />
                <small
                  >{{ metric.passed }} 通过 · {{ metric.failed }} 失败 ·
                  {{ metric.not_applicable }} 不适用<span v-if="metric.errors">
                    · {{ metric.errors }} 错误</span
                  ></small
                >
              </article>
              <article class="metric-card gate-card">
                <span>发布门槛</span
                ><strong>{{ Math.round(report.release_gate.minimum_score * 100) }}%</strong
                ><small>{{ gateReason[report.release_gate.reason_code] }}</small>
              </article>
            </div>
            ...
            <div class="report-grid">
              <article class="report-panel">
                <div class="panel-title">
                  <h3>全部检查结果</h3>
                  <el-tag type="danger" plain>{{ failed.length }} 项失败</el-tag>
                </div>
                <div
                ...
                >
                  <div class="result-head">
                    <span
                      ><b>{{ caseNames[item.case_id] }} · {{ item.evaluator_name }}</b
                      ><small>{{ item.reason }}</small></span
                    >
                    <el-tag :type="outcomeType(item.outcome)" size="small">{{
                  ...
                  <ul v-if="item.checks.length" class="check-list">
                    <li v-for="check in item.checks" :key="check.id">
                      <span>
                        ...
                        <small
                          v-if="check.expected !== null || check.actual !== null"
                          class="expected-actual"
                          >期望 {{ JSON.stringify(check.expected) }} · 实际
                          {{
                            check.actual_missing ? '字段不存在' : JSON.stringify(check.actual)
                          }}</small
                        >
                      </span>
                      <el-tag :type="outcomeType(check.outcome)" size="small" effect="plain">{{
                        outcomeText[check.outcome]
                      }}</el-tag>
                    </li>
                  </ul>
                  <button
                    v-if="item.outcome === 'fail'"
                    class="trace-link"
                    @click="openTrace(item.case_id)"
                  >
                    查看失败轨迹 →
                  </button>
                </div>
                ...
                <el-table :data="runs" empty-text="暂无运行" size="small">
                  <el-table-column label="Agent" min-width="190"
                    ><template #default="scope">{{
                      scope.row.manifest.target.ref.external_version_id
                    }}</template></el-table-column
                  >
                  <el-table-column prop="status" label="状态" width="95" />
                  <el-table-column label="操作" width="70"
                    ><template #default="scope"
                      ><el-button link type="primary" @click="openRun(scope.row.id)"
                        >查看</el-button
                      ></template
                    ></el-table-column
                  >
      ...
      <template v-if="trace">
        ...
        <div v-if="traceTurns.length" class="trace-turns">
          ...
          <el-card v-for="(turn, index) in traceTurns" :key="turn.turnId" shadow="never">
            ...
            <small>轮次结束状态</small>
            <pre>{{ JSON.stringify(turn.state, null, 2) }}</pre>
          </el-card>
        </div>
        <h3>执行轨迹</h3>
        <el-timeline
          ><el-timeline-item
            ...
            ><el-card shadow="never"
              ><b>{{ span.name }}</b
              ><el-tag size="small">{{ span.operation_type }}</el-tag
              ><small v-if="span.attributes.turn_id">轮次 {{ span.attributes.turn_id }}</small>
